...
function Aside() {
    ...
    return (
        <aside id="price" className='md:my-28 relative'>
            <Image alt='background' className='-bottom-36 h-[200px] absolute -z-10' src={background} />
            <div className='flex items-center justify-center'>
                <Image width={90} height={90} src={crystallLeft} alt='crystallLeft' />
                <h3 className='font-semibold text-xl md:text-4xl '>Choose your plan</h3>
                <Image width={90} height={90} src={crystallRight} alt='crystallRight' />
            </div>
            <p className='text-center leading-7 md:text-lg lg:leading-10 lg:text-2xl xl:leading-10 xl:text-3xl text-gray my-7 font-medium'>Choose your plan and enjoy all the features you need for an exceptional educational experience for your children.</p>
                ...
                {data.map(item => (
                    <Link href={"/"} className='hover:scale-95 transition-all duration-300 py-8 px-5 rounded-xl bg-white shadow-[0_0px_50px_-5px_rgba(0,40,100,0.25)]' key={item.id}>
                        <div className='text-center'>
                            <h3 className='text-xl md:text-2xl	capitalize mb-4 font-semibold'>{item.title}</h3>
                            <h4 className='text-3xl md:text-4xl	font-bold'>${item.price}</h4>
                            {item.about !== "" ? (<p className='text-lg md:text-xl mt-5'>{item.about}</p>) : (<div className='mb-12'></div>)}
                        </div>
                        <Image className='mx-auto' alt={item.title} src={item.image} />
                        <ol style={{ "listStyleType": "inherit" }} className='mt-10 px-5'>
                            {item.details.map((detail, index) => (
                                <li className='my-4 md:font-medium marker:text-[#0066FF] text-lg' key={index}>{detail}</li>
                            ))}
                        </ol>
                    </Link>
                ))}
